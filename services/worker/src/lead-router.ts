...
export async function routeInboundLead(input: RouteInboundLeadInput): Promise<RouteInboundLeadResult> {
  ...
  if (matchedRule.assignType === "USER") {
    ...
    // Fetch user info for the selected pool member
    if (assignedUserId) {
  ...
  try {
    await prisma.chatConversation.update({
      ...
      data: {
        ...
        status: "HANDOFF", // handoff so agents can take over
      },
    });
  } catch (err) {
  ...
  // 7. Send direct WhatsApp notification to the assigned agent if enabled and agent has phone
